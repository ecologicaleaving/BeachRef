...
- **DONE**: Issue #22 — Security hardening Supabase: RLS su 6 tabelle pubbliche, views ricreate come SECURITY INVOKER, search_path fisso su 30+ funzioni, policy restrittive su matches/sync_error_log/analytics_events, accesso materialized view ristretto. ⚠️ **Rettifica dalla #77**: "policy restrittive" valeva per **`INSERT` e `UPDATE`, non per `SELECT`** — la migration 016 è applicata e funziona, ma dice nel proprio commento (§3c) *"Keep read access, restrict writes to service_role"*, e cinque tabelle interne (`sync_error_log`, `alert_rules`, `sync_status`, `referee_analytics`, `analytics_events`) sono rimaste leggibili da chiunque avesse la anon key fino alla #77. Una policy protegge un **verbo**, non una tabella: dichiararla per tabella è ciò che ha reso invisibile il buco
...
- **TODO** (emerso da #45, **deciso dalla #54 fase 2 — AC12**): `services/DataConsistencyValidator.ts` e `services/DataSyncService.ts` sono **rimasti orfani** dopo la rimozione della catena `Migration*` — erano importati solo da lì. **Decisione: vanno rimossi entrambi; nessuno dei due serve alla fase 3.** Non rimossi nella PR della fase 2 perché è un diff di ~1.550 righe (più due suite di test) estraneo al meccanismo di attivazione, e mescolarlo renderebbe illeggibile la review del gate: va fatto come commit/issue a sé. Le ragioni, in ordine di gravità:
  - **`DataConsistencyValidator` non può girare nella web app, per costruzione.** Il suo costruttore fa `createClient(EXPO_PUBLIC_SUPABASE_URL, process.env.EXPO_PUBLIC_SUPABASE_SERVICE_ROLE_KEY!)`: una **service_role key** dentro una variabile `EXPO_PUBLIC_*`, cioè inlinata nel bundle JavaScript pubblico. Farlo funzionare significherebbe pubblicare la chiave di amministrazione del database — l'esatto contrario di quello che la fase 1 sta verificando. È anche il motivo per cui non è un "orfano innocuo": è una mina che aspetta che qualcuno configuri quella variabile per farla esplodere. Se il drift detection serve, il suo posto è una Edge Function, dove la service_role key è legittima perché non attraversa mai il browser. Importa inoltre `node:crypto`, che su RN/web non esiste.
  - La #71/#73 ha già accertato che è **scritto contro un contratto mai esistito** (`VisApiClient` non ha `getInstance()`, `NetworkMonitor` non era esportato da `NetworkStateManager`): non è codice funzionante e dormiente, è codice che non ha mai girato.
  - **`DataSyncService` scrive**, non legge. La fase 3 riguarda le **letture** DB-first; la scrittura verso Supabase è compito del cron di sync lato server (migration 003). Il suo `getInstance()` inoltre fa `createClient(...!)` e `startScheduler()` nel costruttore — la stessa forma che ha tenuto inerte il ramo DB per caso anziché per scelta. Esiste anche un secondo orfano omonimo, `services/sync/DataSyncService.ts`, senza alcun importatore
- **DONE**: Issue #54 **fase 2** (la fase 1 e la fase 3 restano aperte) — Il meccanismo di attivazione controllata. Prima di questa PR "Supabase è configurato" e "l'app legge dal DB" erano **lo stesso fatto**: `DualReadService` è cablato su `readStrategy: 'db_first'` e il suo costruttore chiamava `createClient(process.env.EXPO_PUBLIC_SUPABASE_URL!, ...)`, che senza la variabile lancia `supabaseUrl is required.` **nel costruttore**. Quel throw era l'unico motivo per cui il ramo DB è inerte oggi — quindi il giorno in cui le due variabili compaiono su Netlify, `getTournaments`/`getMatches`/`getReferees`/`clearCache` cambiano sorgente dati per tutti insieme. **Cosa c'è ora**: `services/flags/DbReadFlags.ts`, un gate **per dominio di lettura** (tornei / match / arbitri) risolto a runtime su 5 livelli — kill switch automatico, query param `?dbReads=`, override persistito, default d'ambiente `EXPO_PUBLIC_DB_READS`, e infine **niente acceso**. Rollback istantaneo senza toccare variabili né ri-deployare: `?dbReads=off` oppure `__beachrefDbReads.off()` dalla console. **Il default è spento**, quindi con le variabili presenti l'app continua a leggere dalla VIS esattamente come oggi (provato con `EXPO_PUBLIC_SUPABASE_*` valorizzate: i test girano tutti in quella configurazione). **Il fallback è stato simulato, non dedotto**: `DualReadService` ora costruisce il client **lazy** e `shouldTryDatabase()` risponde `false` se manca la configurazione, così `db_first` degrada invece di esplodere; e `dbTimeoutMs` — che esisteva in `DualReadConfig` da sempre ed era applicato a **nulla**, mentre solo `apiTimeoutMs` raggiungeva una richiesta — ora limita davvero ogni query, quindi un Supabase **lento** (non solo irraggiungibile) cede il passo. Scoperta collaterale: il ramo "API" *interno* a `DualReadService` **non è la VIS**, è una Edge Function dietro `EXPO_PUBLIC_EDGE_URL` (non configurata); il fallback vero alla VIS è sempre vissuto nei **chiamanti** di `CacheServiceCompatibility` e si raggiunge rigettando — per questo il gate rigetta invece di restituire dati vuoti. Rimosso `EXPO_PUBLIC_USE_NEW_HOOKS`, secondo interruttore all-or-nothing che nessuno impostava. Aggiunto `scripts/verify-rls-anon.ts` (`npm run verify:rls`) per la **fase 1**: interroga il DB di produzione con la sola anon key, rifiuta di partire senza chiave e **rifiuta una service_role key** (decodifica il claim `role` del JWT), sonda 25 tabelle in lettura/insert/update/delete in modo non distruttivo e stampa la tabella dell'AC1. tsc **1708** e lint **901 (5 errori)** in parità con master, suite jest identiche + 3 nuove verdi (35 test), `audit:ci` PASS
...
- **TODO**: **`/tournament-teams` non mostra ancora nessuna squadra.** La #73 ha tolto il crash (`getTournamentTeamList is not a function`) ma la feature di `specs/003-players-entry-list` resta a metà, e il motivo è che **la spec descrive una risposta VIS che la VIS non dà**. Sondato dal vivo: `GetTournamentTeamList` esiste e risponde 200, ma restituisce `<TournamentsTeams><TournamentTeam No= Version= />`, **non** `<BeachTeams><BeachTeam>` con attributi camelCase come assume `data-model.md` e come parsa `TournamentTeamService.parseTeamListResponse`; ignora inoltre sia `No` sia `Fields` (ha risposto con tutte le 3286 righe). L'endpoint che porta davvero la entry list è **`GetBeachTeamList`** con `<Filter NoTournament="…"/>`, verificato: `<BeachTeams><BeachTeam No= Name="A/B" NoTournament= />`. Il blocco è che l'app porta un numero di **evento** (`NoEvent`, es. 1734) mentre quel filtro vuole un numero di **torneo** (valori tipo 21), e un evento contiene più tornei. **Non ri-derivare**: è costato diversi sondaggi, tutto è nel doc comment di `VisApiClient.getTournamentTeamList`
- **DONE (migration da applicare a mano)**: Issue #77 — Il DB di produzione è **deny-all** per `anon` e `authenticated`. Non "policy più strette": nessun privilegio su nessun oggetto di `public`, RLS su ogni tabella, **tutte** le policy ereditate cancellate, default privileges revocati (una tabella creata domani nasce chiusa) e un blocco di verifica dentro la migration stessa che **rifiuta il commit** se qualcosa sopravvive. La scelta è deny-all perché **nessuno consuma quel DB dall'API pubblica**: il web non ha le variabili Supabase configurate e l'app Flutter non è mai stata distribuita — non c'era alcun consumatore da preservare, e ogni concessione lasciata in piedi sarebbe diventata un permesso che fra due anni nessuno saprebbe più motivare. **AC5 — perché la #22 non produceva l'effetto dichiarato**: la migration 016 *è* applicata e *funziona* (provato: un `INSERT` anonimo su `matches` viene rifiutato con `42501`, che è esattamente `matches_service_insert`), ma copre `INSERT`/`UPDATE` e **lascia intatto `SELECT`, dichiarandolo in un commento**. Le cinque tabelle ancora leggibili lo erano per policy `FOR SELECT USING (true)` scritte nelle migration 004/009/011 per una dashboard di monitoraggio **mai costruita**. Nessun regresso, nessuna migration saltata: una dichiarazione fatta nell'unità sbagliata (la tabella) rispetto a quella che PostgreSQL usa (il verbo), e **nessuno che avesse mai tenuto in mano la chiave pubblica e guardato**. **La prima misura era però sbagliata a sua volta, e in eccesso**: RLS su `SELECT`/`UPDATE`/`DELETE` **filtra righe, non rifiuta istruzioni**, quindi lo script dell'AC1 — che filtrava su un id impossibile per non fare danni — otteneva "accettato" su ogni tabella qualunque fosse la policy. Da lì i "17 tabelle scrivibili": `matches` risultava cancellabile pur **non avendo alcuna policy di DELETE**. Stessa cosa in lettura: un `SELECT` negato torna `[]`, non un errore, quindi 7 delle 12 "tabelle private leggibili" non erano accertate. **Reali e accertati**: 5 tabelle interne davvero leggibili (righe e colonne tornate indietro), 4 tabelle di dominio leggibili senza che nessuno l'avesse mai deciso (`referees` compresa, 12 colonne di dati personali), e **`tournaments` che accetta un `INSERT` anonimo** — accertato perché la riga vuota arriva fino a `23502 not-null`, cosa che accade solo *dopo* che il `WITH CHECK` è passato. **AC4** — i due `PGRST204` erano un bug della sonda, non un risultato: `sync_status` e `schema_versions` non hanno affatto una colonna `id` (chiavi `entity_type` e `version`), quindi PostgREST rifiutava il **filtro** e nulla raggiungeva il DB; stessa famiglia dei `22P02` su altre 8 tabelle. `scripts/verify-rls-anon.ts` riscritto: chiave primaria dichiarata per tabella, filtro `<pk> IS NULL` (falso per ogni riga, e type-agnostico), quarto verdetto **`inconclusive`** che **fa fallire il run** — perché sotto deny-all ogni rifiuto è un `42501` esplicito sollevato prima del match delle righe, e l'ambiguità sparisce. Le quattro proprietà di RLS su cui si regge tutto questo sono asserzioni eseguibili in `supabase/tests/rls_deny_all.test.sql` (A2–A5), che verifica anche la 017 su un PostgreSQL vero in Docker: **tutte verdi**. **AC8** — `.github/workflows/rls-verify.yml`: `verify:rls` gira ogni giorno, su ogni PR che tocca `supabase/**`, e a mano; **se i secret mancano il job fallisce, non salta**. Rimosso `BeachRef-Complete-Deploy/` (4,9 MB di build obsoleto tracciato su repo pubblica, con dentro la anon key). Doc: **`supabase/RLS.md`**. ⚠️ **La migration NON è stata applicata al DB di produzione** — lo fa Davide; finché non è applicata `npm run verify:rls` resta a exit 1, ed è corretto che lo sia
- **DONE**: Issue #60 — Scope dell'audit **differenziato per checker** su `supabase/functions` (Edge Function Deno), non binario. `security` **continua** ad analizzarle: sono codice di prima parte che gira in produzione e maneggia dati, ed è il checker che lì vale di più (#56). Escluse solo dai cinque checker che codificano assunzioni Expo/React Native (`architecture`, `error-handling`, `performance`, `data-flow`, `build`); `typescript`/`complexity`/`eslint` sono agnostici e restano ammessi dalla config (il `tsconfig.json` le esclude già dalla #49, `complexityRoots`/`lintRoots` non le elencano). Meccanismo nuovo: `AUDIT_CONFIG.checkerExcludePaths`, **additivo** a `excludePaths` — passare un checker id può solo escludere di più, mai riaprire un path escluso globalmente. Ogni esclusione porta una `reason` che il run **stampa** prima di partire (blocco `Scope:`), coerentemente col principio della #42. **Baseline 1795 → 1741**, di cui solo **−5** dovuti alla #60 (3 `error-handling-promise` sulle Edge Function + 2 `typescript-error` su `netlify/`, che era in `excludePaths` da sempre ma non veniva mai applicato ai finding TypeScript); gli altri −49 erano scarto di una baseline non rigenerata dopo #71/#73. `audit:ci` **PASS / exit 0**, verificato anche sul master risultante dal merge. tsc **1708, invariato**; lint **901, invariato**. ⚠️ **Il vero difetto trovato**: il security scanner **non leggeva le Edge Function affatto**. `getFilesToScan()` si fermava a 500 file — 516 su 801 di prima parte, e **zero** sotto `supabase/`, perché il cap tagliava le directory di primo livello in ordine alfabetico dopo `services/`. Un segreto finto piantato in una Edge Function **non veniva segnalato**. Cap rimosso e sostituito da un guard rail che **solleva** (`SCAN_FILE_LIMIT`): una scansione credenziali troncata non deve poter dire "nessun segreto". Tolti anche due falsi positivi che ne derivavano (separatori Windows in `isInTestOrComment`, e valori `${...}`/`Deno.env` scambiati per credenziali hardcoded)
- **TODO**: Rientro del baseline `.audit-baseline.json` verso zero (1741 finding bloccanti dopo #60: 1706 TS, 5 ESLint error, 30 error-handling; **0 Critical, 0 security**)
