...
- **TODO**: `screens/RefereeSettingsScreen.tsx` (rotta `/referee-settings`) usa **`selectedDate`, `setSelectedDate`, `VisApiService`, `Tournament` mai dichiarati** — 34 `TS2304`, di cui alcuni nel corpo di render: `ReferenceError` garantito. Stessa classe della #71
- **TODO**: **Sei metodi chiamati su `VisApiClient` che non esistono** — `fetchMatchesForTournament`, `fetchBeachTournamentsThisYear`, `getTournaments`, `getMatches`, `getEvents`, `getTournamentTeamList` (19 `TS2339`), da `useCourtManagement`, `useRefereeManagement`, `useRealtimeData`, `RealtimeFallbackService`, `DataConsistencyValidator`, `TournamentTeamService` e da `screens/TournamentDetailScreen.tsx:1749` (**rotta raggiungibile**). `TypeError: not a function` al primo passaggio. Stessa classe della #71
- **TODO**: `screens/MatchDetailScreen.tsx` legge `liveData.closedSets` / `.currentSet` / `.points.a` su un `BeachMatchLiveDTO` che espone invece `score.sets` / `score.currentSet` / `score.points` — **il punteggio del set in corso non viene mai mostrato** su `/match-detail`. Non e' un crash solo perche' `currentSet` e' `undefined` e la guardia va in corto
- **TODO**: `components/entities/Match/MatchCard.tsx` (raggiungibile) usa tre chiavi di stile inesistenti — `setScoreText`, `liveIndicatorText`, `currentSetScoreText` → testo non stilato
