...
## Tasks / Subtasks
- [ ] Implement mobile-first responsive patterns across all shadcn components (AC: 1)
  - [ ] Audit all shadcn components for mobile-first responsive implementation
  - [ ] Enhance TournamentTable responsive behavior for mobile networks
  - [ ] Optimize TournamentRow card layout for small screens and slow connections
  - [ ] Implement responsive navigation patterns for mobile tournament usage
- [ ] Verify and enhance touch target compliance for tournament glove usage (AC: 2)
  - [ ] Audit all interactive elements for 44px minimum touch target compliance
  - [ ] Enhance button sizing and spacing in view toggle controls
  - [ ] Optimize card interaction areas for gloved touch usage
  - [ ] Implement proper touch feedback and haptic-like visual responses
- [ ] Create high contrast theme options for outdoor tournament visibility (AC: 3)
  - [ ] Develop high contrast color variants for FIVB theme
  - [ ] Implement theme toggle functionality for outdoor/indoor usage
  - [ ] Enhance text contrast ratios for bright sunlight readability
  - [ ] Add high contrast variants for all shadcn Alert, Badge, and Card components
- [ ] Implement offline-ready UI states for poor venue connectivity (AC: 4)
  - [ ] Create offline detection and state management system
  - [ ] Implement offline-first data caching for tournament information
  - [ ] Design offline UI indicators and user guidance
  - [ ] Integrate offline states with existing error handling and retry mechanisms
